...
/**
 * Clase que se encarga de la conexion con el servidor y el trafico de paquetes de informacion (en Desktop, Android,
 * iOS)
 *
 * socket: Conexion con el servidor.
 * thread: Segundo hilo de ejecucion para escuchar respuestas constantemente.
...
public class GnConnection implements Connection {
	...
	public boolean connect() {
		...
		try {
			...
			Dialogs.showOKDialog(Main.getInstance().getBundle().get("error"), "No se pudo establecer la conexion con el servidor.");
			socket = null;
			return false;
		}
		return false;
