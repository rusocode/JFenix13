...
public class ServerPackages {
	...
	// Enumeracion de los paquetes que se reciben del servidor
	enum ID {
		Logged, // LOGGED
		RemoveDialogs, // QTDL
		...
		UserOfferConfirm, CommerceChat, ShowBlacksmithForm, // SFH
		ShowCarpenterForm, // SFC
		UpdateSta, // ASS
		UpdateMana, // ASM
		UpdateHP, // ASH
		UpdateGold, // ASG
		UpdateBankGold, UpdateExp, // ASE
		ChangeMap, // CM
		PosUpdate, // PU
		ChatOverHead, // ||
		...
		CharacterCreate, // CC
		CharacterRemove, // BP
		CharacterChangeNick, CharacterMove, // MP, +, * and _ '
		ForceCharMove, CharacterChange, // CP
		ObjectCreate, // HO
		ObjectDelete, // BO
		BlockPosition, // BQ
		PlayMusic, // TM
		...
		SendNight, // NOC
		Pong, UpdateTagAndStatus, SpawnList, // SPL
		ShowSOSForm, // MSOS
		ShowMOTDEditionForm, // ZMOTD
		ShowGMPanelForm, // ABPANEL
		UserNameList, // LISTUSU
		UpdateStrenghtAndDexterity, UpdateStrenght, UpdateDexterity, AddSlots, MultiMessage, StopWorking, CancelOfferItem, SubeClase, ShowFormClase,
		EligeFaccion, ShowFaccionForm, EligeRecompensa, ShowRecompensaForm, SendGuildForm, GuildFoundation, UpdateUsersOnline
	}
	...
	/**
	 * Ejecuta los metodos correspondientes al ID de los paquetes recibidos.
	 */
	public void handleReceived(byte[] bytes) {
		/*
		 * Esta variable se activa si se recibe un paquete que no existe (para que deje de procesar paquetes, sino se rompe el
		 * juego).
		 */
		...
		try {
			while (r.getAvailable() > 0 && !broken) {
				...
				// Marco la posicion del comienzo del paquete
				r.mark();
				...
				case UpdateUsersOnline:
					...
					break;
				default:
					// Si llega un paquete que no esta implementado...
			...
			/*
			 * Es comun que un paquete llegue cortado, por lo que no hay suficientes datos para leer...
			 * entonces se vuelve hasta la posicion marcada (comienzo del ultimo paquete).
			 */
			r.reset();
		}
	}
	...
	public void handleChangeInventorySlot() throws NotEnoughDataException {
		...
		item.set(r.readShort(), r.readString(), r.readShort(), r.readBoolean(), r.readShort(), r.readByte(), r.readShort(), r.readShort(),
				r.readShort(), r.readShort(), r.readFloat());
	...
	/**
	 * Guarda el index con el que el servidor conoce usuario que estamos manejando.
	 */
	...
	private void handlePlayMusic() throws NotEnoughDataException {
		int num = r.readByte();
		if (num > 0) getAssets().getAudio().playMusic(num);

		// Corresponde a los loops, pero considero que todas las musicas se repiten infinitamente
		r.readShort();
	}

	/**
	 * Define la nueva area, y borra todos los personajes y objetos que no pertenecen a esa area.
	 */
	private void handleAreaChanged() throws NotEnoughDataException {
		// Los valores estan hardcodeados
	...
	/**
	 * Crea un PJ o NPC, asignandole todas sus caracterasticas y lo ubica en una posicion del mapa.
	 */
	private void handleCharacterCreate() throws NotEnoughDataException {
		...
		if (privs != 0) {
			...
			if ((privs & 64) != 0 && (privs & 1) == 0) privs = (byte) (privs ^ 64);

			// Si es del concejo de banderbill y tiene privilegios
			if ((privs & 128) != 0 && (privs & 1) == 0) privs = (byte) (privs ^ 128);

			// Si es rolmaster
			if ((privs & 32) != 0) privs = 32;
	...
	private void handleCharacterChange() throws NotEnoughDataException {
		...
		if (index == u.getIndexInServer() && u.isCambiandoDir()) u.setCambiandoDir(false);
	...
	private void handleLogged() {
		if (Gdx.app.getType() == Desktop || Gdx.app.getType() == WebGL) Main.getInstance().setScreen(new PrincipalView());
		else Main.getInstance().setScreen(new PrincipalViewM());
	}
	...
	private void handleRemoveCharDialog() throws NotEnoughDataException {
		...
		// esto es realmente necesario? (fijarme).. ya que como el diálogo es un atributo del char, si borro el char se pierde
		// el diálogo
	}
