...
public class ClientPackages {
	...
	// Enumeracion de los paquetes que se envian al servidor
	enum ID {
		...
		UserCommerceConfirm, CommerceChat, BankEnd, // FINBAN
		...
		ForumPost, // DEMSG
		MoveSpell, // DESPHE
		MoveBank, UserCommerceOffer, // OFRECER
		Online, // /ONLINE
		Quit, // /SALIR
		...
		Denounce, // /DENUNCIAR
		Ping, // /PING
		GMCommands, InitCrafting, Home, Consulta, RequestClaseForm, EligioClase, EligioFaccion, RequestFaccionForm, RequestRecompensaForm,
		EligioRecompensa, RequestGuildWindow, GuildFoundate, GuildConfirmFoundation, GuildRequest, MoveItem
	}

	/* Las ID de los comandos de GM empiezan en 0, ero deberian empezar en 1!,
	...
	public byte[] removeAll() {
		...
		/* Guarda el tama駉 para asegurarse que va a extraer solo lo que hay hasta este punto, y evitar procesar algo
	...
	/**
	 * Agrega el array pendiente a la cola, y lo vac铆a para mas escritura de paquetes
	 */
	public void write() {
		if (w.getSize() > 0) {
	...
	public void writeLoginNewChar(String name, String password, String mail, int raza, int sexo, int ciudad) {
		...
		for (int i = 0; i < 22; i++) {
			if (i == 0) w.writeByte(10);
			else w.writeByte(0);
		}
	}

	/**
	 * Caminar hacia una direcci贸n
	 */
	...
	/**
	 * Cambio de direcci贸n sin cambiar de tile
	 */
	...
	/**
	 * Petici贸n de posici贸n (para corregirla)
	 */
	...
	public void writeMoveItem(int slot1, int slot2) {
		...
		w.writeByte(slot1);
		w.writeByte(slot2);
		w.writeByte(1); // TODO: tipo (inventario, banco, etc...) no se maneja todav铆a en el servidor...
	}
