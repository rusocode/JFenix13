...
/**
 * Touchpad que controla el movimiento del personaje (se usa en dispositivos moviles)
 *
 * tpSkin y tpStyle: necesarias para darle un estilo al controlador. world: referencia al mundo para poder moverse en
 * él.
 */
public class Controller extends Touchpad {
	...
	public void act(float delta) {
		...
		if (isTouched()) if (!w.isMoving()) {
			float despX = getKnobPercentX();
			float despY = getKnobPercentY();
			Config.Direccion dir = null;
			if (despX != 0 || despY != 0) if (Math.abs(despX) > Math.abs(despY)) if (despX > 0) dir = ESTE;
			else dir = OESTE;
			else if (despY > 0) dir = NORTE;
			else dir = SUR;

			if (dir != null) w.moveChar(dir);
		}
	}
}
