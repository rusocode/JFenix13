...
public abstract class Grid<T extends Slot> extends VisTable {
	...
	public void setSlot(T slot, int index) {
		...
		if (selected == index + 1) setSelected(slot);
	}
	...
	public int indexOf(Slot slot) {
		...
		for (int i = 0; i < cells.size; i++)
			if (cells.get(i).getActor() == slot) return i + 1;
		return -1;
	}
	...
	public void setSelected(int index) {
		...
		s = getSelected();
		if (selected != -1 && s != null) s.setChecked(false);
	...
	protected ClientPackages getClPack() {
		return Main.getInstance().getConnection().getClPack();
	}
